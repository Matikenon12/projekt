...
class Explosion:
    def __init__(self, x, y, typ="maly"):
        ...
        #Pusty hitbox zeby gra nie wywalala bledu przy szukaniu pustego miejsca na powerupy na mapie
        ...
        #Wybor odpowiednich wspo³rzêdnych z arkusza w zaleznosci od typu (rozmiaru) wybuchu
        if self.typ == "maly":
...
def sprawdz_wszystkie_kolizje(gra):
    ...
    #1.Zderzenia pocisków miedzy soba
    ...
    #2.Zderzenia pociskow z map¹ i czolgami
    for pocisk in pociski:
        if not pocisk.aktywny: 
            continue

        #Poza map¹ MALY WYBUCH
